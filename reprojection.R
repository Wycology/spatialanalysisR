...
# Last modified on 18th December 2022   #
#########################################
...
# st_is_longlat is one grea function for checking if geographic CRS is in use

# Here we start by a dummy dataframe and check for crs

london <- data.frame(lon = -0.1,
                     lat = 51.5) %>% # Creating a data.frame, and then
  sf::st_as_sf(coords = c('lon', 'lat')) # Setting variables lon and lat as coords
