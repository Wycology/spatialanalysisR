...
Loading the necessary packages to use in the codes which will be helpful in analyzing geographic data in R. font_import() helps to make the fonts nrought by `extrafont` available to R for use.
...
There is a lot of information which we get by loading the data including path to the data-set being loaded, driver which has been used to load the file, geometry type, dimension, bounding box, and CRS. We can get information about the sf object just like with normal data-frame or tibble, for example:

```{r structure}
str(honeybee)
```

This is telling us that we are working with a point feature. Such a cool information to have. Now we may need to know the coordinate reference system of our data. This is a very important step before running any analyses on the data as it has effect on the units of measurements.

```{r coordinaes}
st_crs(honeybee)
```

This is returning a super helpful information. We can also have the information within the text as `r st_crs(honeybee)`. I still have no idea how the output will look like. Let us see by knitting the markdown. Everything thrown to text.
...
This seams to be fine even though I know pretty well the lines cut themselves at some point(s). Let me try this with a poygon feature. I will create two polygons, one kind of a rectangle and the other a funny shape which cuts itself. I think that the latter should be an invalid polygon. Here we go.
...
Now let me check for the validity of the features I have in my data-set of the dummy_poly
...
Before I proceed to honeybee work, I want to create a point sf with some points having exactly the same coordinates and see whether such as considered intersecting. This can help me with duplicates of occurrence records in species distribution modeling later.
...
Then I check for validity of the data which I know has four points with one being duplicated.

```{r duplicate_points}
sum(st_is_valid(points_sf) == TRUE)
```

Okay, I conclude that this validity is not helpful in case of duplicate points as all my four points are regarded valid. So, I may be right to say that duplicate points are not intersecting features, maybe.

Now back to honeybee work. I want to project it to UTM zone 15N for Minnesota. I remember that earlier, while working with geocomputation with R, I developed a function which can help with knowing the epsg code for any place on earth provided you know long and lat of the place, which I can get from Google Earth search for Minnesota.
...
This is returning `r lonlat2UTM(c(-94.685798, 46.729741))` which is different from 26915 which has been used in the [tutorial](https://www.katiejolly.io/rladies-spatial/). Okay, they are using different datums or data :). 26915 is NAD.

Now I can project the honeybee data to my desired epsg code.
...
Now to create a plot of these points, we may need to have a background map. Otherwise just points on a white background will not make much sense. So I will import some free background map and assign them same projection as the points.
...
I will load the necessary extrafont package as I prepare to use ggplot for the plotting of the map and use some unique font in title lable.
...
It is quite unfortunate that I still cannot tell how spatial join works. I need to study the output deeply. This is calling for joining the two datasets into one. The type of join to use here is left join which is the default. Loosely speaking, I will say
